...
2. [F7.5](docs/tasks/completed/F7.5.md): produto PR #94/head `008bd69`/CI `33332733192`/merge `c82e099`/pós-merge `33333106708`; admin local. [F7.C1](docs/tasks/completed/F7.C1.md) terminal.
...
17. [DEC-014](docs/decisions/DEC-014-reconciliacao-pos-merge.md),
    [DEC-015](docs/decisions/DEC-015-composicao-canonica-fase4.md), [DEC-016](docs/decisions/DEC-016-composicao-operacional-antes-da-release.md) e [regras dos agentes](.agents/AGENTS.md).

## 2. Estado atual

| Campo | Estado observado |
|---|---|
| **Fases concluídas** | Fases 0–6; F7.1–F7.5 promovidas; reconciliação final F7.5 pendente |
| **Fase ativa** | Fase 7 — reconciliação terminal e publicação da RC |
| **Tarefa ativa** | Nenhuma tarefa ativa de produto; reconciliação administrativa F7.5 em curso |
| **Gate** | `PROMOTED / ADMIN_PR_OPEN` — produto F7.5 certificado em `main` |
| **Estado corrente** | Produto #94 terminal; admin #95/head inicial `9640089`/CI `33333639460` verde; head documental final, merge, tag e prerelease pendentes |
...
| **Branch** | `docs/promote-f7.5`, criada do merge certificado `c82e099427c9d2fb4a4ce8f3af635769202293f2` |
...
| **Checkpoint F7.C1** | `checkpoint/f7.c1-ready` → `caf1dad`; produto `2415f41`; `checkpoint/f7.c1-complete` → `f1d8cd0`; PR #92/head final `bb4dc8c`/CI `33325055342`/merge `26c36ff`/pós-merge `33325679613`; checkpoints somente locais |
| **Gate F7.5** | package `0.2.0rc1`; tag futura `v0.2.0-rc.1`; GitHub prerelease após merge/pós-merge; sem PyPI, F8, `1.0` ou remoção de refs |
...
|---|---|
| Tarefa | F7.C1 — composição operacional e reconciliação terminal |
| Produto | PR #92/head `bb4dc8c`/CI `33325055342`/merge `26c36ff`/pós-merge `33325679613` |
| Reconciliação administrativa | PR #93/head final `c99f02e`/CI `33326822791`/merge `3415c385` |
| CI terminal | `33327198301` passou onze jobs obrigatórios mais `CI required` no SHA exato `3415c385` |
| Fronteira | F7.5 desbloqueada; RC `0.2.0rc1` congela metadata/docs/limitações e release sem expandir produto ou F8 |
...
Nenhuma tarefa ativa de produto. A [F7.5](docs/tasks/completed/F7.5.md) está `PROMOTED`: PR #94,
head `008bd69`, CI `33332733192`, merge `c82e099` e pós-merge `33333106708` verdes. A reconciliação
administrativa está no PR #95/head inicial `9640089`/CI `33333639460`; tag e prerelease aguardam seu fechamento.

## 6. Bloqueios e fronteiras externas

Não há bloqueio técnico. O sandbox local exige TEMP confinado e acesso explícito ao cache uv; o mesmo
E2E passou fora dele. PyPI, F8, `1.0`, macOS certificado, force/bypass e remoção de refs permanecem
fora do escopo. Tag publicada será imutável; correção posterior exige nova RC.

## 7. Próxima ação exata

```text
PUBLICAR O REGISTRO NO PR #95 E EXIGIR CI FINAL, MERGE E PÓS-MERGE VERDES.
NÃO PUBLICAR TAG ANTES DOS MERGES, CIs PÓS-MERGE E RECONCILIAÇÃO TERMINAL VERDES.
```

## 8. Retomada após perda de contexto

1. Leia `.agents/AGENTS.md`, este painel, `docs/tasks/completed/F7.5.md`, a Fase 7 e as DEC-014/DEC-016.
...
*Atualizado em: 2026-08-30T17:35:00-03:00 | Fonte: F7.5 produto terminal; admin #95/9640089/33333639460 verde*
