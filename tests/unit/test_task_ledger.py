...
def test_f5_7_promotion_preserves_r3_negative_evidence_and_certification() -> None:
    ...
    f7_c1_dossier = _read(COMPLETED_ROOT / "F7.C1.md")
    task_index = _read(TASKS_INDEX)
    readme = _read(ROOT / "README.md")
    ...
    for evidence in (
        ...
        "9fdd3cde2accfd40211252ab884b7f1091f341ce",
        "31899279536",
    ...
    assert "261f0977f9d0ed16ac51ce569b631a43ae7e49ff" in f6_4_dossier
    ...
    assert "> **Reconciliação administrativa:** `ADMIN_PR_OPEN / CHECKS_PENDING`" in f6_3_dossier
    ...
    for evidence in (
        ...
        "31913877551",
        "https://github.com/Wf-ops1/xXHarnessinfraXx/pull/74",
        "c92597111f0c9ad11c01360f0348357c2fe379b2",
        "31916819934",
    ):
    ...
    assert "> **Reconciliação administrativa:** `ADMIN_PR_OPEN / CHECKS_PENDING`" in f6_1_dossier
    assert "docs/promote-f6.1" in panel
    assert "docs/promote-f6.1" in task_index
    for source in (panel, task_index, f6_1_dossier, readme):
        assert "45b7f03" in source
        assert "PR #70" in source or "pull/70" in source
        assert "aae1aea" in source
        assert "31888260797" in source
    ...
    assert "> **Lifecycle:** `PROMOTED`" in f7_c1_dossier
