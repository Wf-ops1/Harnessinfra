...
está concluída e certificada somente na branch local; a promoção, a composição automática F7.C1 e a
F7.5 continuam pendentes. Execute esses comandos somente em cópias descartáveis.
...
|---|---|---|---|
| Ambiente e pacote | `uv.lock`, build de wheel, metadata SPDX Apache-2.0, defaults via `importlib.resources` e smoke externo ao checkout | F7.4 está somente local; bootstrap ainda depende de instalar `uv`, e macOS não possui job de CI | Distribuição pública e instalação externa suportadas como produto |
...
| CI e release | GitHub Actions executa quality/tests/package em Windows e Linux; F7.3 tornou mypy strict, coverage/branches, secrets e auditoria de dependências gates obrigatórios; a F7.4 passou integralmente no ambiente local | A F7.4 ainda não possui CI de PR; composição pública F7.C1 e release candidate F7.5 não começaram | Distribuição pública e processo de release operacional na F7 |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  workflow `31888260797`; o head final `9a346bd7` passou 11/11 no run `31888564163`, foi incorporado
  pelo merge `ac887b0` e recebeu 11/11 na CI pós-merge `31888960272` em 5m29s. Pela DEC-014, esse
  encerramento é terminal. A F6.2 está `PROMOTED`; a branch de produto
  `task/f6.2-harden-journal`: o probe comprovou o writer paralelo; a implementação passou a usar o
  journal canônico com checkpoint HMAC opcional e exports fail-closed. O focado passou
  `146 passed in 27.87s`; o full passou `954 passed, 5 skipped, 6 subtests passed in 471.73s`; quality,
  ...
  `PROMOTED` na branch `task/f6.3-evidence-manifest`: `7` testes dedicados,
  `253` focados e a full R2 com `965 passed, 5 skipped, 6 subtests passed`; wheel e smoke offline
  estão verdes. O produto é `0bf3a59`, com o checkpoint `checkpoint/f6.3-complete`. O head final
  `ed1f0e0` do [PR #73](https://github.com/Wf-ops1/xXHarnessinfraXx/pull/73) passou 11/11 no run
  `31913438082`, foi incorporado pelo merge `1bd095a` e recebeu 11/11 na CI pós-merge `31913877551`
  ...
  `31929031317` em 5m41s. A reconciliação administrativa
  [PR #76](https://github.com/Wf-ops1/xXHarnessinfraXx/pull/76) encerrou no head final `3c1f4d2`,
  passou 11/11 no run `31930869377`, foi incorporada pelo merge `a42ec411` e recebeu 11/11 na CI
  pós-merge `31931649225`. A F6.5 passou `172` focados e full final
  `1023 passed, 5 skipped, 6 subtests`, além de quality, build e smoke offline. O head final
  ...
  `31956649961`. A F6.6 concluiu no head `1ce953d`, com focado `234` e full R2
...
- a F5.5 promovida introduz `RedactionContext` imutável/repr-safe, remove fallback secreto dos adapters
